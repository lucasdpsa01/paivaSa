# Site Currículo
Um web site, onde mostro minhas skills, projetos, falo um pouco sobre mim, e uma área de contato.

## Tecnologias
Este projeto foi desenvolvido com as seguintes tecnologias:
- React.js + TypeScript
- Vite
- HTML, CSS e TypeScript
- Bibliotecas (React Router, Email.js)

## Evolução do projeto
Este projeto começou em JavaScript puro e foi migrado para TypeScript, mantendo toda a funcionalidade e o visual originais.
- 🏷️ [`v1`](https://github.com/lucasdpsa01/paivaSa/releases/tag/v1) — versão original em JavaScript
- 🏷️ [`v2.0.0`](https://github.com/lucasdpsa01/paivaSa/releases/tag/v2.0.0) — migração completa para TypeScript

## Demonstração
Modo claro
<img width="2552" height="1237" alt="Captura de tela 2026-06-10 121456" src="https://github.com/user-attachments/assets/548a8fd3-7317-4d02-af90-5f5b7a68c5e1" />
Modo Escuro
<img width="2552" height="1230" alt="Captura de tela 2026-06-10 121515" src="https://github.com/user-attachments/assets/7fcabe24-6d10-4964-8bf6-af4605637b76" />

## Como rodar o projeto
## 🔧 Pré-requisitos
Antes de começar, certifique-se de ter instalado:
- Node.js e npm ou yarn

## 🔨 Passo a passo
1) Clone este repositório:
```git clone https://github.com/lucasdpsa01/paivaSa.git ```
2) Acesse a pasta
```bash cd sla```
3) Instale as dependências:
```npm install```
4) Inicie o servidor de desenvolvimento:
```npm run dev```
5) Abra o navegador na porta que o Vite indicar

## Estrutura do Projeto
```
📦 sla
|- 📂src
|  |- 📂components
|  |- 📂pages
|  |- 📂assets
|  |- 📂data
|  |- 📂routes
|- 📜 .gitignore
|- 📜 eslint.config.js
|- 📜 index.html
|- 📜 package-lock.json
|- 📜 package.json
|- Readme.md
|- 📜 tsconfig.json
|- 📜 vite.config.ts
````

## 🛠 Funcionalidades
- ✅ Criar comentários e receber comentários na caixa de entrada do email
- ✅ Responsivo
- ✅ Tipagem estática com TypeScript

## 🤝 Contribuição
Sinta-se à vontade para contribuir! Para isso:
1. Faça um fork do projeto.
2. Crie uma branch com a sua feature: ```git checkout -b minha-feature```
3. Faça o commit das suas alterações: ``` git commit -m 'Adicionando nova feature' ```
4. Faça um push para a sua branch: ``` git push origin minha-feature ```
5. Abra um Pull Request.
